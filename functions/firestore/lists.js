...
exports.sendPushMessage = f.firestore.document('lists/{itemId}').onWrite(async (change, context) => {
  const listData = change.after.data()
  ...
  for(const userId of commentData.pushUserList) {
    // 書き込んだ本人には通知を送らない
    if (listData.last_updated_by === userId) continue
