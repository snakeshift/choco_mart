...
export default {
  ...
  methods: {
      ...
      switch(item.status){
        case(this.STATUS.NOT_RECRUITING): {
          ...
          if(this.is_empty(item.name) || this.is_empty(item.price)){
            ...
            this.dialog.content = `
              【${item.name}】を${item.price}<br>で出品します。取引場所を選択してください。
            `
            ...
            this.dialog.button.positive.func = async () => {
              ...
              await this.registerSellList({item, index, target: this.dialog.target.value})
              this.dialog.isShow = false
    ...
    ...mapActions('firebase', ['getSellList', 'registerSellList', 'closeSellList', 'setSellListPrice']),
    ...mapMutations('firebase', ['setSellList'])
  },
  computed: {
    TYPE: () => TYPE,
    STATUS: () => STATUS,
